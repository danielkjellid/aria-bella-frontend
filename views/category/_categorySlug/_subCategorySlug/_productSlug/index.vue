          ...
          <div class="info-box bg-white rounded-md">
            <div class="px-5 py-5">
              ...
              <div v-else class="mt-5">
                <h2 v-if="product.display_price" class="text-lg font-semibold text-gray-900">kr {{ product.from_price | formatPrice }} <span class="text-gray-600">per {{ product.unit }}</span></h2>
              </div>
              <div v-if="variants" class="mt-5">
                ...
                <div v-else class="grid grid-cols-8 gap-3 mt-3">
                  <button v-for="variant in variants" :key="variant.id" @click="selectVariant(variant.id)">
                    <div class="relative">
                      <img
      ...
      <section class="mt-12">
        <div>
          <ProductDescriptionBlock
            :title="product.name"
            :price="{gross_price: product.from_price, unit: product.unit}"
        ...
        <div v-if="variants.length > 0" class="mt-12">
          <ProductVariantBlock
            title="Varianter"
            :loaded="!fetchState.pending"
            :variants="variants"
        ...
        <div v-if="sizes.length > 0" class="mt-12">
          <article>
          ...
          <article class="mt-12">
            ...
            <table v-else class="lg:w-3/4 w-64 mt-6">
              <tbody>
                ...
                <ProductSpecRow title="Fasonger" :fields="product.shapes" />
  ...
  setup() {
    ...
    **************/
    ...
    interface ProductObj {
      ...
      options: ProductOptionObj[]
    }

    interface ProductOptionObj {
      id: number
      variant: ProductMetaObj[]
      size: ProductMetaObj[]
    }
    ...
    ***********/
    ...
    const { fetch, fetchState } = useFetch(async () => {
      product.value = await $axios.$get(`products/product/${slug}/`)
